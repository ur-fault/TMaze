use std::io::stdout;

use cmaze::dims::Dims;
use tmaze::{
    renderer::{CellContent, GBuffer},
    settings::theme::{Color, NamedColor, Style, TerminalColorScheme},
    ui::Rect,
};

fn main() {
    let scheme = TerminalColorScheme::named("catppuccin_mocha");

    let mut buf = GBuffer::new(Dims(64, 32));

    for i in 0..64 {
        buf.mut_view().fill_rect(
            Rect::sized_at(Dims(i, 0), Dims(1, 32)),
            CellContent::styled(
                ' ',
                Style {
                    bg: Some(Color::Named(NamedColor::Red)),
                    alpha: (256 / 64 * i) as u8,
                    ..Default::default()
                },
            ),
            &scheme,
        );
    }

    for i in 0..32 {
        buf.mut_view().fill_rect(
            Rect::sized_at(Dims(0, i), Dims(64, 1)),
            CellContent::styled(
                '$',
                Style {
                    bg: Some(Color::Named(NamedColor::Black)),
                    fg: Some(Color::Named(NamedColor::White)),
                    alpha: (256 / 32 * i) as u8,
                    ..Default::default()
                },
            ),
            &scheme,
        );
    }

    buf.write(&mut stdout()).unwrap();

    // let red = Style {
    //     fg: Some(Color::Named(NamedColor::Black)),
    //     bg: Some(Color::Named(NamedColor::Red)),
    //     ..Default::default()
    // };

    // let mut buf = GBuffer::new(Dims(4, 2));
    // buf.write(&mut stdout()).unwrap();
    //
    // println!("----");
    // buf.mut_view().clear();
    // buf.write(&mut stdout()).unwrap();
    //
    // println!("----");
    // buf.mut_view().fill(CellContent::styled(' ', red), &scheme);
    // buf.write(&mut stdout()).unwrap();
    //
    // println!("----");
    // buf.mut_view().clear();
    // buf.write(&mut stdout()).unwrap();
    //
    // println!("----");
    // buf.mut_view().fill(
    //     CellContent::styled(
    //         ' ',
    //         Style {
    //             bg: Some(Color::Named(NamedColor::Red)),
    //             alpha: 10,
    //             ..Default::default()
    //         },
    //     ),
    //     &scheme,
    // );
    // buf.write(&mut stdout()).unwrap();
    //
    // println!("----");
    // return;

    // let f = |buf: &mut GBuffer, alpha| {
    //     buf.mut_view().inside(|f| {
    //         f.fill(CellContent::styled('あ', red), &scheme)
    //             .centered(Dims(6, 2), |f| {
    //                 f.fill(
    //                     CellContent::styled(
    //                         '$',
    //                         Style {
    //                             bg: Some(Color::Named(NamedColor::Blue)),
    //                             alpha,
    //                             ..Style::default()
    //                         },
    //                     ),
    //                     &scheme,
    //                 );
    //             });
    //     });
    //     buf.write(&mut stdout()).unwrap();
    // };
    //
    // let mut buf = GBuffer::new(Dims(14, 6));
    //
    // f(&mut buf, 255);
    //
    // buf.mut_view().clear();
    // f(&mut buf, 128);
    //
    // buf.mut_view().clear();
    // f(&mut buf, 0);
}

