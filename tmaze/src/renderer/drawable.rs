...
pub trait Drawable {
    ...
    fn draw_force_style(&self, pos: Pos, frame: &mut Frame, style: ContentStyle);
}
...
impl Drawable for String {
    ...
    fn draw_force_style(&self, pos: Pos, frame: &mut Frame, style: ContentStyle) {
        self.as_str().draw_force_style(pos, frame, style);
    }
}
...
impl<D: Drawable> Drawable for (D, ContentStyle) {
    fn draw(&self, pos: Pos, frame: &mut Frame) {
        self.0.draw_force_style(pos, frame, self.1);
    }

    fn draw_force_style(&self, pos: Pos, frame: &mut Frame, style: ContentStyle) {
        self.0.draw_force_style(pos, frame, style);
    }
}
